...
func TestIncrement(t *testing.T) {
	...
	ok := Increment(tc, "tint", 2)
	if !ok {
		t.Error("Error incrementing")
	}
	x, found := tc.Get("tint")
	if !found {
...
func TestDecrement(t *testing.T) {
	tc := New[string, int](DefaultExpiration, 0)
	tc.Set("tint", 3, DefaultExpiration)
	ok := Decrement(tc, "tint", 1)
	if !ok {
		t.Error("Error decrementing int:")
	}
	x, found := tc.Get("tint")
	if !found {
		t.Error("tint was not found")
